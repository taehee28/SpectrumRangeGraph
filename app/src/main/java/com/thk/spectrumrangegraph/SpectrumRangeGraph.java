...
/**
 * 총 범위를 표시하고, 범위 내에서의 최대/최소치와 평균값을 표시하는 그래프
 *
 * 1. 그래프의 높이는 'srg_graphHeight'값에 6dp를 추가한 값이 된다.
 *
 * 2. 그래프의 'android:layout_height' 속성은 'wrap_content'로 설정 가능하고,
 *    'match_parent' 혹은 '0dp'(ConstraintLayout의 경우) 혹은 특정 값을 입력하면 부모 View의 크기만큼 늘어남.
 *
 * 3. 코드상에서 레이아웃 사이즈를 설정하는 경우, 픽셀값으로 설정해야 함. (setGraphHeight, setTextSize, setTextMarginTop)
 *
 */
